...
// Companies array
const companies = [
  { name: "TechCorp", color: "bg-blue-700" },
  { name: "Innovex", color: "bg-pink-700" },
  { name: "DigiTrend", color: "bg-blue-600" },
  { name: "NextWave", color: "bg-pink-700" },
  { name: "FutureLab", color: "bg-blue-600" },
];
...
function Testimonials() {
  ...
  return (
    <section className="bg-black py-16 px-4 md:px-20 text-center">
      ...
      <div className="relative max-w-5xl mx-auto" id="testimonials">
        <div className="bg-[#2f2f2f] rounded-3xl shadow-md p-8 md:p-12 flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-6 text-left">
          ...
          <div>
            ...
            <p className="text-lg text-white mb-4">"{testimonials[current].message}"</p>
      ...
      {/* Trusted Companies */}
      <div className=" text-white py-10 mt-12 rounded-xl">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {companies.map((company, index) => (
            <div
              key={index}
              className="shadow-md rounded-md bg-zinc-800 px-6 py-3"
            >
              <span
                className={`text-white font-semibold px-4 py-2 rounded ${company.color}`}
              >
                {company.name}
              </span>
            </div>
